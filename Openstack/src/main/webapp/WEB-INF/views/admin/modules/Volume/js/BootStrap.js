...
function setup() {
	...
	$(panel).delegate("[name='filter_volumename'],[name='filter_volumezone'],[name='filter_volumeowner']", "keyup", function() {
...
function filter(panel) {
	var rows=$(panel).find(".volumeRow");
	var nameprefix=$.trim($(panel).find("[name='filter_volumename']").val().toLowerCase());
	var zoneprefix=$.trim($(panel).find("[name='filter_volumezone']").val().toLowerCase());
	var ownerprefix=$.trim($(panel).find("[name='filter_volumeowner']").val().toLowerCase());
	
	var count=0;
	for(var i=0; i<rows.length; i++) {
		var volumename=$(rows[i]).find("input[zmlm\\\:item='volumeName']").val().toLowerCase();
		var volumezone=$(rows[i]).find(".volumeZone").html().toLowerCase();
		var volumeowner=$(rows[i]).find(".volumeOwner").html().toLowerCase();
		
		if(0!=volumename.indexOf(nameprefix) || 0!=volumezone.indexOf(zoneprefix) || 0!=volumeowner.indexOf(ownerprefix)){
			$(rows[i]).hide();
		}else{
			$(rows[i]).show();
		...
		success: function(data) {
			...
			try{
				...
				switch(data.status) {
					...
					case "attachedalready": msg=Locale["volume.message.attachedalready"]; break;
					case "error": ;
...
function showAttachVolumeDialog(which) {
	...
	var vmid=$(which).parents(".volumeRow").first().find("input[zmlm\\\:item='volumeVm']").val();
	var volumename=$(which).parents(".volumeRow").first().find("input[zmlm\\\:item='volumeName']").val();
	var volumeowner=$(which).parents(".volumeRow").first().find("input[zmlm\\\:item='volumeOwner']").val();
	...
	loadVm(zone, volumeowner, list);
	
}

function loadVm(zone, volumeowner, container) {
	$.ajax({
		type: "POST",

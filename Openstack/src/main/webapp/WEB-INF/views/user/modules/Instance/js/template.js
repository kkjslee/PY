...
var Template_VmRow=[
'<span class="vmRow">',
	'<input zmlm:item="vmId" type="hidden" value="${vd}" />',
	'<input zmlm:item="vmName" type="hidden" value="${name}" />',
	...
	'{{else}}',
		'<span class="vmRowCell vmName textCollapse">${name}</span>',
		//'<span class="vmRowCell vmZone textCollapse">${zonedisplay}</span>',
		...
		'<span class="vmRowCell vmOperation textCollapse">',
			...
			//'{{if getRole()=="senior"}}',
				'<a href="#" onclick="showCloneDialog(this);return false;">'+Locale["vm.template.vm.clone"]+'</a>',
...
'<div id="${id}" class="vmDetail">',
	'<table style="width:100%;">',
		...
		'<tr>',
			...
			'<td name="accesspoint">',
				'<a name="javavnc" target="_blank" href="#" style="color:#ff6f10;">',
					'<span class="ui-icon ui-icon-extlink" style="float:left; margin:0 0 0 0;"></span>',
					Locale["vm.template.info.vnc.access"].sprintf("VNC"),
				'</a>',
				', ',
				'<a name="webvnc" target="_blank" href="#" style="color:#ff6f10;">',
					Locale["vm.template.info.vnc.access"].sprintf("Web"),
				'</a>',
			'</td>',
		'</tr>',
		'<tr>',
			...
			'<td name="vmpasswd">',
				'<a onclick="showVmPasswd(this);return false;" href="#" style="color:#ff6f10;">',
					...
					Locale["vm.template.info.link.getvmpasswd"],
				'</a>',
			'</td>',
		'</tr>',
		'<tr>',
...
var Template_ClonePanel=[
'<div id="${id}" title="'+Locale["vm.dialog.title.clone"]+'" class="clonePanel" >',
	'<div style="text-align:center;background:none;">',
		'<table>',
			'<tbody>',
				'<tr class="vmtemp"><td class="label">'+Locale["vm.template.clone.vmtemp"]+'</td><td class="value"></td></tr>',
				'<tr class="vmname">',
					'<td class="label">'+Locale["vm.template.clone.vmname"]+'</td>',
					'<td class="value"><input placeholder="'+Locale["vm.template.clone.vmname.placeholder"]+'" class="inputVmname" maxlength="<!--#echo var="MaxLength_Instance"-->" /></td>',
				'</tr>',
				'<tr class="vmzone"><td class="label">'+Locale["vm.template.clone.vmzone"]+'</td><td class="value"></td></tr>',
				'<tr class="vmcpu"><td class="label">'+Locale["vm.template.clone.vmcpu"]+'</td><td class="value"></td></tr>',
				'<tr class="vmmem"><td class="label">'+Locale["vm.template.clone.vmmem"]+'</td><td class="value"></td></tr>',
				'<tr class="vmsoft"><td class="label">'+Locale["vm.template.clone.vmsoft"]+'</td><td class="value"></td></tr>',
				'<tr class="vmplan"><td class="label">'+Locale["vm.template.clone.vmplan"]+'</td><td class="value"></td></tr>',
				'<tr class="vmprice"><td class="label">'+Locale["vm.template.clone.vmprice"]+'</td><td class="value red"></td></tr>',
				'<tr><td colspan="2" style="text-align:center;"><span class="vice">'+Locale["vm.template.clone.vmprice.tips"]+'</span></td></tr>',
			'</tbody>',
		'</table>',
	'</div>',
'</div>'

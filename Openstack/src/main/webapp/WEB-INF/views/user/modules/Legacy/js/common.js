...
function loadVmplan() {
	$.post(
		...
		function(data) {
			...
			var ext_str='';
			if(v_vmplanId.indexOf('PFM')>-1 && v_vmplanId.indexOf('year')==-1) {//包月
			...
			}else if(v_vmplanId.indexOf('year')>-1) {//包年
				createInfo('#buyInfos_4', Locale["inst.page.info.yearly.fee"], Locale["inst.page.info.yearly.fee.format"].sprintf($(dataObj).find('SubscriptionFee').text()));
				v_paymentAmount=$(dataObj).find('SubscriptionFee').text();

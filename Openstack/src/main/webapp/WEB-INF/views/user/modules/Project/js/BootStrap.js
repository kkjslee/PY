...
function showProcessingDialog() {
	var view=$("<div style='text-align:center;'><img src='css/image/progress.gif'/>"+Locale["project.dialog.processing"]+"</div>").dialog({
		autoOpen: true,
		width: 240,
		height: 100,
		resizable: false,
 ...
 */
function removeProject(projectid, container) {	
	if(confirm(Locale["project.confirm.removeapply"])) {
		...
		$.ajax({		
			...
			cache: false,
			data: {
				operation: "deleteprojectinstance",
...
function summary(container) {
	var zone=container.find("#zone").val();
	var zonename=container.find("#zone").find("option[value='"+zone+"']").html();
